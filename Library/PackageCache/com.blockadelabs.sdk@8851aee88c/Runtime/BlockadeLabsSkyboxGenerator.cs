...
namespace BlockadeLabsSDK
{
    public class BlockadeLabsSkyboxGenerator : MonoBehaviour
    {
		...
		private string folderPath = "Assets/MyGames/Default";
        ...
        public async void GenerateSkyboxAsync(string worldName, string pathToFolder)
        {
			gameName = worldName;
            ...
            try
            {
				...
				// ================== MESH CREATION ==================
				// Extract the mesh from the skybox object using the Genesis package modifications
				var skyboxObject = _skybox.gameObject;
				var meshCreator = skyboxObject.GetComponent<MeshCreator>();
				meshCreator.ExtractMesh(folderPath);
...
#if UNITY_EDITOR
        private static string CreateGenerateFolder(string name, string path)
        {
            var generateFolder = "Assets/MyGames";
            if (!AssetDatabase.IsValidFolder(generateFolder))
            {
                AssetDatabase.CreateFolder("Assets", "MyGames");
            }

            // Create a folder to store the new assets
            var uniquePath = AssetDatabase.GenerateUniqueAssetPath(path);
			Debug.Log("unique: "+uniquePath);
        ...
        private async Task DownloadResultAsync(GetImagineResult result)
        {
			...
			Debug.Log("attempting: " + folderPath);
			Debug.Log("prefix: " + prefix);
			Debug.Log("prompt: " + prompt);
            var filePath = CreateGenerateFolder(gameName, folderPath);
            var texturePath = filePath + "/" + prefix + "_main_texture.png";
            var depthTexturePath = filePath + "/" + prefix + "_depth_map_texture.png";
            var resultsPath = filePath + "/" + prefix + "_data.txt";
			...
			// After we downloaded the files, we need to change the shader to genesis, set read/write enabled, and set the depthMap format to R8
			...
			var importerDepth = TextureImporter.GetAtPath(depthTexturePath) as TextureImporter;
			Preset depthPreset = Resources.Load("DepthMapPreset") as Preset;

			depthPreset.ApplyTo(importerDepth);
            importerDepth.SaveAndReimport();
            ...
            AssetDatabase.CreateAsset(material, filePath + "/" + prefix + "_material.mat");
            AssetDatabase.Refresh();
        ...
        private Material CreateMaterial(Texture2D texture, Texture2D depthTexture, int remixId)
        {
			...
			material.SetFloat("_Max", 20);
            material.mainTexture = texture;
            if (material.HasProperty("_DepthMap"))
            {
				Debug.Log("Setting depth map");
